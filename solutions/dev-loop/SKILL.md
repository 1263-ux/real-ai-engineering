...
- 目的：防止自审，不是制造流程税；必须 fresh context，默认便宜/白嫖模型，最多一次；修复后允许一次针对修复项的增量复核，不计入重复审查。
...
- **变更边界**：提交前锁定本次变更白名单（仅任务涉及文件）；`.agent/`、docs、未跟踪文件与用户历史变更不入提交；功能修复不与文档/临时文件混合提交（项目分支规则优先）。
...
- 工具/浏览器超时 ≠ 业务失败；超时后查最终状态（页面内容、会话记录、服务日志、实际返回）再判定。
