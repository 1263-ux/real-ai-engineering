...
- **分级执行（L0–L3）**：小任务不套完整流程。L0 一行修改（文案/CSS/解释）→ 改+快速验证+汇报；L1 普通代码（单模块/一两个文件）→ 简短计划+改+测试；L2 运行态/跨模块（插件、Host-Client、配置写回、服务重启、环境）→ 完整流程+独立检查+受控运行态验证；L3 外部交付（commit/push/PR/发布/外部消息）→ 完整流程+Owner 对具体动作逐一授权。
- **风险优先于文件数量**：涉及真实数据 / 服务重启 / 认证 / 权限 / 协议 / 外部发布的任务，即使只改一个文件，最低按 L2/L3 执行。
- **时间盒**：根因判断 ≤15 分钟、可运行修复 ≤30 分钟；超时必须停止试错并输出：已确认事实 / 当前假设 / 已排除项 / 阻塞原因 / 下一步所需决策。
- **根因先行**：先给根因再给流程；根因明确后立即执行，不重复核查已确定的事实。
- **独立检查（单次、场景触发）**：仅 易泄露数据 / 改真实配置 / 涉及路径权限发布包 / 用户明确要求 review 时启用，且只一次（默认便宜档）；用户说「直接 / 继续 / 授权」后不得再追加；其余场景以本地验证为准，不默认拉 reviewer。
- **证据够了就停（停止条件）**：用户说「直接 PR / 继续 / 授权 / 快」= 明确停止信号；已通过的检查不重复全量重跑；验证 = 针对性测试（+必要时 build/pack/扫描）+ 一次状态确认，够了即交付，不追加 review、不重复拉取、不创建展示性 agent。
- **子 agent 并行（不阻塞主线）**：只读/探索/扫描类子 agent 默认后台——搜泄露路径/密钥、查无界扫描/路径遍历、查上游规范、汇总测试失败、耗时库探索。主代理在 方案明确 / 针对性测试已过 / 用户已说「直接 PR」 时不等待，直接提交推送；子 agent 晚到发现 BLOCKER → 单独补 fix commit，而非等待。
- **可阻塞主线的三种情况**：① 确认是否泄露密钥/个人数据；② 确认是否破坏真实用户配置/数据；③ 下一步确实依赖其答案（如该用哪个协议/API）。除此之外不等待。
- **已授权 PR 硬规则（最多）**：1 次局部 diff + 1 次针对性测试 + 1 次敏感扫描 + 1 次 commit/push + 1 次 PR 状态确认。PR 主体已存在、本轮只补局部改动（上限/路径语义/测试等）时按「追加小修」处理：局部 diff → 针对性测试 → 敏感扫描 → commit → push → 一次确认；不重套完整 L3、不重跑已冻结测试、不重新梳理 PR 全文、不补无价值工件。
...
- 每轮只更新一次工件，避免反复写 .agent 文件造成额外开销；状态滞后时下轮开工先同步一次。
...
- 独立检查单次、按场景触发（易泄露数据 / 改真实配置 / 路径权限发布包 / 用户要求）；用户停止信号优先；稀缺升级按价值判断（宪法 7）走 G3，与上限（最多一次）构成边界。
...
- 运行态动作（重启服务 / 改真实数据 / 外部发布）在计划阶段就请求一次明确授权，授权须含：具体动作 / 目标服务 / 影响范围 / 回滚方式 / 是否允许修改真实数据；不要等源码流程全跑完才发现需要重启/发布。
...
- 冻结是有条件的：已通过且 FROZEN 的路径在条件不变时不重复验证；以下情况使 FROZEN 失效——源码/依赖变化、运行环境变化、验收数据变化、核心闭环失效、用户报告与已通过证据矛盾。
